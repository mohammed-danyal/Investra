...
const Holdings = () => {
  const { holdings: allHoldings, loading } = useContext(PortfolioContext);
  const [chartType, setChartType] = useState("bar");

  if (loading) return <p className="title">Loading holdings...</p>;
  if (!allHoldings || allHoldings.length === 0) {
    return (
      <div className="orders" style={{ height: "60vh" }}>
        <h3 className="title">Holdings (0)</h3>
        <div className="no-orders">
          <p>You don't have any holdings yet.</p>
          <p style={{ fontSize: "0.85rem", color: "#aaa", marginTop: "10px" }}>
            Go to the watchlist on the left, hover over a stock, and click <strong>Buy</strong> to place an order!
          </p>
        </div>
      </div>
    );
